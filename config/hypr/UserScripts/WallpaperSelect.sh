...
if command -v swww >/dev/null 2>&1 && command -v swww-daemon >/dev/null 2>&1; then
  ...
  daemon_process="swww-daemon"
  daemon_format="xrgb"
elif command -v awww >/dev/null 2>&1 && command -v awww-daemon >/dev/null 2>&1; then
  wallpaper_cmd="awww"
  daemon_cmd="awww-daemon"
  daemon_process="awww-daemon"
  daemon_format=""
else
  notify-send -i "$iDIR/error.png" "Wallpaper" "Install swww or awww first"
  exit 127
fi
...
if (( ${#PICS[@]} == 0 )); then
  notify-send -i "$iDIR/error.png" "Wallpaper" "No wallpapers found in $wallDIR"
  exit 1
fi
...
modify_startup_config() {
  ...
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    ...
    sed -i '/^\s*exec-once\s*=\s*.*ApplyWallpaper\.sh.*$/s/^/\#/' "$startup_config"
    ...
    sed -i '/^\s*#\s*exec-once\s*=\s*.*ApplyWallpaper\.sh.*$/s/^\s*#\s*//;' "$startup_config"
...
apply_image_wallpaper() {
  ...
  if ! pgrep -x "$daemon_process" >/dev/null; then
